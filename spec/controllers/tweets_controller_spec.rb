...
RSpec.describe TweetsController, type: :controller do
  ...
  describe "POST #create" do
    ...
    let(:valid_attributes) { attributes_for(:goodmorning, user_id: user.id)}
  ...
  describe '#PATCH #update' do
    let(:tweet) { create(:goodevening) }
    let(:new_attributes) { attributes_for(:goodmorning) }
    let(:invalid_attributes) { attributes_for(:invalid_tweet) }
    ...
    context 'with valid params' do
      it 'updates the selected tweet' do
        ...
        tweet.reload
        expect(tweet.message).to eq("Good Morning")
